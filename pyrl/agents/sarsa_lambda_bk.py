...
class sarsa_lambda(skeleton_agent):
    ...
    def agent_init(self,taskSpec):
        ...
        self.numActions = 4**4 # (256 actions)
        actions = (-0.25, -0.05, 0.05, 0.25)
        self.intToRealAction = []
        for i in range(4):
            for j in range(4):
                for k in range(4):
                    for l in range(4):
                        self.intToRealAction.append((actions[i], actions[j], actions[k], actions[l]))
        ...
        if self.fa_name == 'fourier':
            ...
            self.basis = tilecode.TileCodingBasis(self.numStates, TaskSpec.getDoubleObservations(),
                                    num_tiles=self.params.setdefault('tile_number', 4),
                                    num_weights=self.params.setdefault('tile_weights', 4608))
        else:
    ...
    def agent_start(self,observation):
        ...
        returnAction=Action(None, 4)
        real_actions = self.intToRealAction[thisIntAction]
        for i in range(4):
            returnAction.doubleArray[i] = real_actions[i];
    ...
    def agent_step(self,reward, observation):
        ...
        returnAction=Action(None, 4)
        real_actions = self.intToRealAction[newIntAction]
        for i in range(4):
            returnAction.doubleArray[i] = real_actions[i];
